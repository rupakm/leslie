...
| **8.5b** | C1 + C2 closure (state-machine surgery) + `corruptLocalInv` weakening + variant redesign | Drop `p ∉ s.corrupted` from gates of `partyEchoSend`/`partyReady`/`partyAmplify` (C1) and from `partyEchoReceive`/`partyReceiveReady` (C2). Widen `partyEchoSend`'s broadcast filter to cover all receivers (C2). Weaken `corruptLocalInv` to drop the `{echoSent, echoesReceived, readySent, readyReceived} = ∅` clauses (now schedule-dependent for corrupt parties). Update `coalitionView_corrupt_factors_AE` to give the weaker (`coeffs`-content-only) conclusion. Update `buildCorruptLocalState`/`reconstructCoalitionTraceView` to take a `coalitionTrivialView` argument (forward-looking infrastructure landed via PR #58). Update `coalitionTraceView_eq_reconstruct_AE` to factor through `(coalitionAlgebraicView, coalitionTrivialView)`. **Variant-analysis dilemma** (deeper analysis 2026-05-05): the AST framework's `FairASTCertificate` has TWO load-bearing variant clauses: `V_super` (variant non-increase across *any* gated action) and `V_super_fair` (variant strict-decrease across *fair* actions).  Once corrupt parties may fire send actions (C1), the original honest-only `unsentEchoSet`/`notReadySentSet` make `V_super` *fail* for corrupt-fired sends — `inflightEchoes` grows by ≤ n, `unsentEchoSet` (honest-only) is unchanged, so `avssU` *increases*.  Three resolution paths, each with trade-offs: **(a)** Split the action constructor into honest `partyEchoSend p` (in `avssFairActions`) and adversarial `partyAdvEchoSend p` (NOT in `avssFairActions`) — but `V_super` is for *all* gated actions, including non-fair ones, so this *doesn't* fix `V_super` for adversarial sends; would additionally need a different `V` function (e.g., `V := avssU + (n+1)·K^4·\|{corrupt p : ¬echoSent}\|`) so adversarial sends drop V, but this requires the correction term to be 0 at terminated, which forces `terminated` to require all-corrupt-party `echoSent = true`, defeating the purpose of the constructor split.  **(b/b'')** Extend `unsentEchoSet := {p : ¬echoSent}` and `notReadySentSet := {p : ¬readySent}` (drop honest filters) so `avssU` strictly decreases on every send (honest *or* corrupt) — `V_super` and `V_super_fair` both work uniformly.  Then **`U_term` requires extending `terminated`** to include `∀ p, echoSent_p = true ∧ ∀ p, readySent_p = true` (so `unsentEchoSet = ∅` at terminated).  This breaks `honestSet_pos_of_not_terminated_pre_share` (the corner-case lemma that lets the `dealerShare` variant decrease lemma close when `honestSet.card = 0`): under the strengthened `terminated`, pre-share state with `honestSet = ∅` is *not* terminated (corrupt parties have `echoSent = false`), so the lemma's contrapositive fails.  Dealing with this corner case requires either reworking `dealerShare`'s strict-decrease analysis or adding `honestSet.card ≥ 1` as an implicit hypothesis (consistent with AVSS's `n ≥ 3t + 1` setting).  **(c)** Modify the `FairASTCertificate` framework itself to have `V_super` return a disjunct (variant non-increase *or* another fair action enabled).  Cleanest semantically but framework-level surgery.  **Decision (deferred to next 8.5b implementation session)**: pick (b'') with rework of `dealerShare`'s variant analysis (smallest cascade if the rework is tractable).  Depends on 8.5a (gate strengthening). | ~300 (revised from ~250 after V_super analysis) | ⏳ pending (forward infra landed via PR #58) |
...
- **PR 8.5 (subdivided)** depends on 8.4's consistency-check infrastructure.  Internal sequencing:
  ...
  - **8.5b** — C1 + C2 + `corruptLocalInv` weakening — depends on 8.5a.
