...
| **8.5b** | C1 + C2 closure (state-machine surgery) + `corruptLocalInv` weakening + variant extension | Drop `p ∉ s.corrupted` from gates of `partyEchoSend`/`partyReady`/`partyAmplify` (C1) and from `partyEchoReceive`/`partyReceiveReady` (C2). Widen `partyEchoSend`'s broadcast filter to cover all receivers (C2). Weaken `corruptLocalInv` to drop the `{echoSent, echoesReceived, readySent, readyReceived} = ∅` clauses (now schedule-dependent for corrupt parties). Update `coalitionView_corrupt_factors_AE` to give the weaker (`coeffs`-content-only) conclusion. Update `buildCorruptLocalState`/`reconstructCoalitionTraceView` to take a `coalitionTrivialView` argument (forward-looking infrastructure landed via this PR — see §17.12 of `AVSS.lean`). Update `coalitionTraceView_eq_reconstruct_AE` to factor through `(coalitionAlgebraicView, coalitionTrivialView)`. **Variant-analysis dilemma** (discovered 2026-05-05): the original `unsentEchoSet`/`notReadySentSet` are honest-only filters; once corrupt parties may fire send actions (C1), the variant-decrease story `avssU_step_partyEchoSend_lt` breaks for corrupt-fired sends.  Naïvely extending `unsentEchoSet := {p : ¬echoSent}` (drop honest filter) lets the variant decrease for corrupt-fired sends but breaks `avssU = 0 at terminated` (corrupt parties may have `echoSent = false` at terminated states).  Resolution choices for 8.5b: (a) split the action constructor into `partyEchoSend p` (honest-only, fair) and `partyAdvEchoSend p` (corrupt-only, *not* in `avssFairActions`) — clean but invasive (requires updating `Fintype`/`Countable` instances + every action case-analysis in simSyncInv preservation, AE structural lemmas, etc.); (b) use `FairASTCertificate.U_dec_det`'s second disjunct (variant decrease *or* another fair action enabled) — every corrupt-fired `partyEchoSend p` enables `partyEchoReceive q p` for any `q`, so the disjunct is satisfied; this requires weakening `avssU_step_partyEchoSend_lt`'s return type and threading the disjunct through `avssCert.U_dec_det`. Depends on 8.5a (gate strengthening). | ~250 (revised from ~200 after dilemma analysis) | ⏳ pending (forward infra landed via this PR) |
