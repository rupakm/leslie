  ...
  | .recvProbeAckAtManager =>
      ...
      · rw [hcur] at hcur'; cases hcur'
        ...
        · -- Dirty probeAck: msg.data = some data → both transferVal and mem set to same value
          ...
          -- After simp: goal is usedDirtySource premise → ... → tx.transferVal = preLines[i].data
          -- (since both new transferVal and new mem = preLines[i].data, simp should close some of it)
          -- If the goal is about the pre-state tx.transferVal:
          intro huds _
          -- From dirtyOwnerExistsInv: ∃ k, dirty ∧ transferVal = preLines[k].data
          obtain ⟨k, hk, _, hk_dirty, htv⟩ := hdirtyOwner tx hcur huds
          -- From preLinesNoDirtyInv: k dirty, i dirty, k ≠ i → contradiction
          -- (dirty → perm .T from preLinesWFInv, then preLinesNoDirtyInv gives perm .N)
